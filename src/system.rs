...
pub fn system_uptime_secs() -> f64 {
    match fs::File::open(PROC_UPTIME) {
        Err(e) => {
            eprintln!("Unable to open the file {} to read uptime: {}", PROC_UPTIME, e);
            0.0
        },
        Ok(mut proc_uptime) => {
            ...
            match proc_uptime.read_to_string(&mut contents) {
                Err(e) => {
                    eprintln!("Unable to read contents of the file {} to read uptime: {}", PROC_UPTIME, e);
                    0.0
                },
...
fn ensure_systemctl(ctlstr: &str, valuestr: &str) {
    ...
    let prev_value_str = ctl.value_string().expect(format!("Unable to read value of {}", ctlstr).as_str());
    if prev_value_str ==  valuestr {
        eprintln!("====> Already enabled, not reenabling: {}", ctlstr);
    } else {
        let real_value_str = ctl.set_value_string(valuestr)
